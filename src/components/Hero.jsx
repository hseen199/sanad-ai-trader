...
const Hero = ({ onConnect }) => {
  ...
  // عند الاتصال بالمحفظة، استدعاء onConnect
  useEffect(() => {
    if (connected && publicKey) {
      onConnect(publicKey.toBase58());
      toast({
        title: t('connected_successfully'),
        description: `${t('wallet_address')}: ${publicKey.toBase58().slice(0, 4)}...${publicKey.toBase58().slice(-4)}`,
      });
    }
  }, [connected, publicKey]);
        ...
        >
          ...
          <div className="flex flex-wrap gap-4">
            ...
            >
              ...
              {connected ? t('enter_dashboard') : t('start_trading_now')}
            </Button>
            <Button
              variant="outline"
              size="lg"
              ...
              onClick={() => {
                window.location.href = '/how-to-start';
              }}
            >
              {t('watch_demo')}
            </Button>
